...
impl PixelRenderer for TerminalRenderer {
    ...
    fn render_frame(&mut self, buffer: &PixelBuffer) -> Result<()> {
        ...
        if self.show_physics {
            // Check if buffer has changed to avoid unnecessary redraws
            let buffer_hash = self.hash_buffer(buffer);
...
impl TerminalRenderer {
    ...
    fn hash_buffer(&self, buffer: &PixelBuffer) -> u64 {
        use std::collections::hash_map::DefaultHasher;
        use std::hash::{Hash, Hasher};

        let mut hasher = DefaultHasher::new();
        // Sample every 16th pixel to make hashing fast
    ...
    fn draw_pixel_buffer_with_viuer(&self, buffer: &PixelBuffer) -> Result<()> {
        ...
        let image_w = 80;
        let image_h = 30;
        let center_x = if term_w > image_w {
            (term_w - image_w) / 2
        } else {
            0
        };
        let center_y = if term_h > image_h {
            (term_h - image_h) / 2
        } else {
            0
        };
...
fn run_visual_test() -> Result<()> {
    ...
    let rgba_bytes: Vec<u8> = renderer
        .buffer
        .pixels
        .iter()
        .flat_map(|pixel| [pixel[0], pixel[1], pixel[2], pixel[3]])
    ...
    let test_configs = vec![
        (
            "🤖 Auto Detection (Let viuer choose)",
            viuer::Config {
                width: Some(80),
                height: Some(30),
                x: center_x,
    ...
    loop {
        ...
        let actually_supported = match name {
            ...
            n if n.contains("Sixel") => {
                ...
                term.contains("mlterm")
                    || term.contains("xterm") && !term_program.contains("Apple_Terminal")
            }
        ...
        let status = if actually_supported {
            ...
            if let Some(rgba_image) =
                RgbaImage::from_raw(renderer.buffer.width, renderer.buffer.height, rgba_bytes)
            {
                let dynamic_image = DynamicImage::ImageRgba8(rgba_image);
            ...
            println!(
                "Mode {}/{}: {} SKIPPED (Terminal doesn't support) | ←→ S=summary Q=quit",
                current_mode + 1,
                test_configs.len(),
                name
            );
            mode_results.push((
                name.to_string(),
                false,
                "Not supported by terminal".to_string(),
            ));
        };
            ...
            if let Ok(Event::Key(k)) = event::read() {
                match k.code {
                    ...
                    KeyCode::Char('q') | KeyCode::Esc => {
                        ...
                        execute!(
                            std::io::stdout(),
                            cursor::MoveTo(0, (test_configs.len() + 5) as u16)
                        )
                        .ok();
                        print!("Exiting...");
                        ...
                        execute!(
                            std::io::stdout(),
                            cursor::MoveTo(0, (test_configs.len() + 6) as u16)
                        )
                        .ok();
...
fn show_summary(results: &[(String, bool, String)], configs: &[(&str, viuer::Config)]) {
    ...
    println!(
        "Summary: {}/{} modes work on {}",
        supported_count, total_count, term_program
    );
    ...
    execute!(
        std::io::stdout(),
        cursor::MoveTo(0, (configs.len() + 3) as u16)
    )
    .ok();
    print!("Press any key...");
    std::io::stdout().flush().ok();
...
fn print_raw_grid() -> Result<()> {
    ...
    for frame in 0..6 {
        ...
        println!(
            "=== Frame {} - t={:.1}s - Drone: x={:.3}m, y={:.3}m, z={:.3}m ===",
            frame + 1,
            sim_time,
            drone_pos[0],
            drone_pos[1],
...
fn main() -> Result<()> {
    ...
    terminal_renderer.log(&format!(
        "Core renderer created - initial drone pos: x={:.3}, y={:.3}, z={:.3}",
